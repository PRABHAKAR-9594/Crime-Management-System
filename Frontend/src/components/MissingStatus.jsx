...
export default function MissingStatusPage() {
  ...
  const currentStageIndex = statusData
    ? statusStages.findIndex(stage => stage.name === statusData.status)
    : 0;

  return (
    <div className="flex flex-col items-center p-8 bg-gray-900 text-white min-h-screen pt-[130px]">
      <div className="w-full max-w-lg p-6 bg-gray-800 rounded-lg shadow-lg">
        <h2 className="text-xl font-semibold mb-4 text-center">Check Missing Report Status</h2>
      ...
      {statusData && (
        <div className="w-full max-w-lg mt-6 p-6 bg-gray-800 border border-red-500 text-white rounded-lg shadow-lg">
          <h2 className="text-xl font-semibold text-center mb-4">Missing Report Details</h2>
          ...
          <div className="relative w-full h-4 bg-gray-700 rounded-full overflow-hidden my-4">
            <div
              className={`absolute top-0 left-0 h-full ${statusStages[currentStageIndex]?.color || "bg-gray-500"} rounded-full transition-all duration-500`}
          ...
          <div className="mt-3 p-4 bg-gray-700 rounded-lg">
            <h3 className="text-lg font-medium text-red-400">Missing Person Information</h3>
            <p><strong>Full Name:</strong> {statusData.missingPerson?.fullName}</p>
            <p><strong>Age:</strong> {statusData.missingPerson?.age}</p>
            <p><strong>Contact:</strong> {statusData.missingPerson?.contact}</p>
            {statusData.missingPerson?.photo && (
              <img src={statusData.missingPerson.photo} alt="Missing Person" className="mt-2 rounded shadow-lg max-w-full h-auto" />
            )}
          </div>

          <div className="mt-3 p-4 bg-gray-700 rounded-lg">
            <h3 className="text-lg font-medium text-red-400">Last Seen Details</h3>
            <p><strong>Location:</strong> {statusData.lastSeenDetails?.location}</p>
            <p><strong>Pincode:</strong> {statusData.lastSeenDetails?.pincode}</p>
            <p><strong>Date:</strong> {new Date(statusData.lastSeenDetails?.date).toLocaleDateString()}</p>
          </div>
        </div>
      )}
    </div>
