  ...
  return (
    <div className="min-h-screen flex bg-black text-white h-[20px]">
      ...
      <aside className="w-1/4 bg-gray-900 pt-[80px]  flex flex-col items-center border-r border-gray-700 " >
        ...
        <NavItem icon={<FaBuilding />} label="Add Office" />
        <Link to={'/viewcriminal'}>
        <NavItem icon={<FaSearch />} label="Search Criminal" />
        </Link>
        <NavItem icon={<FaBriefcase />} label="Take the Case" />
        <NavItem icon={<FaFileAlt />} label="Department Report" />
        <Link to={'/addcriminal'}>
        <NavItem icon={<FaUserSecret />} label="Enter Criminal Report" />
        <NavItem icon={<FaUserSecret />} label="Register Report" />
        </Link>
      ...
      <main className="flex-1 p-10 flex flex-col justify-center items-center">
        <div className="bg-gray-800 bg-opacity-30 backdrop-blur-lg p-8 rounded-lg shadow-2xl border border-gray-600 max-w-3xl w-full text-center">
          <h1 className="text-4xl font-bold text-red-500 mb-6">Welcome to the Department Of Theft</h1>
