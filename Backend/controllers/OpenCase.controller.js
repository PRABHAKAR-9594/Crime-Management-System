...
export const OpenCaseDetails_controller = async (req, res) => {
    const officerUserName = req.body.officerUserName
    try {
        
        const records = await crimeRegFormModel.find({ "assignedOfficer.UserName":officerUserName ,"status":"Under investigation"})
        if (records.length === 0) {
    ...
    export const OpenCaseCloseTicket_controller = async (req, res) => {
        try {
            ...
            const caseExists = await crimeRegFormModel.findOne({ acknowledgeNumber });
            if (!caseExists) {
